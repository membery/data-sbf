(function() {
	...
	var STADIUM_REQ_COLLECTION = 'stadiumRequests';
	...
	function RequestChangedHandler(ctx) {
		...
		*/
		this.handleRequestCreated = function(event, collection) {
			...
			} else if (event.user && event.user.officerBaseball && event.user.officerBaseball.club) {
				entity.requestData.clubApplicant = {
					schema: 'uri://registries/organizations#views/clubBaseball-km/view',
					oid: event.user.officerBaseball.club.oid
				};
			} else if (event.user && event.user.officerSoftball && event.user.officerSoftball.club) {
				entity.requestData.clubApplicant = {
					schema: 'uri://registries/organizations#views/clubSoftball-km/view',
					oid: event.user.officerSoftball.club.oid
				};
			} else {
	...
	*/
	RequestChangedHandler.prototype.handle = function(event) {
		...
		}else if(event.eventType === 'event-stadium-request-created') {
			this.handleRequestCreated(event, STADIUM_REQ_COLLECTION);
		...
		}else if(event.eventType === 'event-stadium-request-updated') {
			this.handleRequestModified(event, STADIUM_REQ_COLLECTION);
	...
	RequestChangedHandler.prototype.getType = function() {
		return ['event-general-request-created', 'event-registration-request-created', 'event-stadium-request-created', 'event-data-request-created', 'event-transfer-request-created',
				'event-general-request-updated', 'event-registration-request-updated', 'event-stadium-request-updated', 'event-data-request-updated', 'event-transfer-request-updated'];
	};
